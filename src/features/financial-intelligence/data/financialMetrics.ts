...
const totalReceita = sum(revenues);
...
const monthlyReceita = [820000, 890000, 1012000];
...
export const financialAnalyticsView: FinancialAnalyticsView = {
  kpis: [
    { icon: "payments", label: "Receita", value: formatCurrency(totalReceita), delta: "+23.4%", tone: "text-status-success", border: "border-secondary", progress: 82 },
    ...
    { icon: "account_balance_wallet", label: "Lucro l?quido", value: formatCurrency(netProfit), delta: "+11.7%", tone: "text-status-success", border: "border-secondary-container", progress: 58 },
    { icon: "sync_alt", label: "Fluxo de caixa", value: formatCurrency(cashFlowTotal), delta: cashFlowTotal >= 0 ? "positivo" : "negativo", tone: cashFlowTotal >= 0 ? "text-status-success" : "text-status-critical", border: cashFlowTotal >= 0 ? "border-status-success" : "border-status-critical", progress: 71 },
    { icon: "stacked_line_chart", label: "Margem bruta", value: `${grossMargin.toFixed(1)}%`, delta: "+2.1pp", tone: "text-status-success", border: "border-outline-variant", progress: Math.min(Math.round(grossMargin), 100) },
    { icon: "receipt_long", label: "Custo operacional", value: formatCurrency(totalExpense), delta: "+6.4%", tone: "text-status-critical", border: "border-status-critical", progress: 44 },
  ],
  revenueEvolution: scaleSeries(monthlyReceita),
  ...
  insights: [
    ...
    { icon: "monitoring", label: "Anomaly Detection", text: "Realizado acima do previsto aparece em materia-prima, frete e servicos administrativos na amostra local.", tone: "text-status-critical", bg: "bg-erro-container/40" },
    { icon: "shield", label: "Risco Alert", text: "Fluxo de caixa positivo na amostra, mas cobertura de liquidez ainda depende de saldo bancario inicial.", tone: "text-status-critical", bg: "bg-status-critical/10" },
    { icon: "stars", label: "Destaque estratégico", text: `Margem bruta simulada em ${grossMargin.toFixed(1)}% com base no arquivo financeiro local.`, tone: "text-status-success", bg: "bg-status-success/10" },
  ],
  tableRows: [
    ["Receitas Operacionais", "Contas", formatCurrency(totalReceita), "+23.4%", "Saudável"],
    ["Lancamentos DRE", "Transactions", `${financialEntries.length} registros`, "+0.0%", "Mapeado"],
    ["Custos e Despesas", "Expenses", formatCurrency(totalExpense), "+6.4%", "Atenção"],
    ["Centros de Custo", "Centros de custo", "4 centros", "+0.0%", "Ativo"],
    ["Resultado Mensal", "Compara??es mensais", formatCurrency(netProfit), "+11.7%", "Adiantado"],
  ],
};
