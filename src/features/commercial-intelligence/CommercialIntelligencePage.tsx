...
import { commercialAnalyticsView } from "./data/commercialMetrics";
    ...
    <>
      ...
      <section className="mt-gutter grid grid-cols-1 gap-gutter xl:grid-cols-12">
        <GlassCard className="p-8 xl:col-span-8">
          ...
          <div className="mt-8 grid grid-cols-1 gap-8 lg:grid-cols-12">
            <div className="lg:col-span-7">
              ...
              <BarChart values={revenueTrend} activeIndex={revenueTrend.length - 1} />
            </div>
          ...
          <div className="mt-8 grid grid-cols-1 gap-6 border-t border-glass-stroke pt-6 md:grid-cols-3">
            <CommercialMetric label="Volume de Venda" value={commercialKpis[1].delta} note="Quantidade UB x terceiros" positive />
            <CommercialMetric label="Ticket Medio" value={commercialKpis[2].value} note="Receita por unidade" bordered />
            <CommercialMetric label="Margem de Venda" value={commercialKpis[4].value} note="Fevereiro/26" />
          </div>
        </GlassCard>

        <GlassCard className="p-8 xl:col-span-4">
          <SectionTitle title="Ranking Clientes" subtitle="Faturamento, participacao e periodicidade" icon="leaderboard" />
          <div className="mt-7 space-y-4">
            {customerRanking.map(([name, revenue, share, recurrence]) => (
        ...
        <GlassCard className="p-8 xl:col-span-7">
          ...
          <div className="mt-8 grid grid-cols-1 gap-6 lg:grid-cols-2">
            ...
            <div className="space-y-4">
              {openOrders.map(([label, value, progress, color]) => (
        ...
        <GlassCard className="p-8 xl:col-span-4">
          ...
          <div className="mt-6 rounded-lg border border-glass-stroke bg-surface-container-low/60 p-4">
              ...
              <tbody className="font-body-sm">
                {commercialLinhas.map(([line, section, value, metric, status]) => (
                  <tr key={line} className="border-b border-glass-stroke/60 transition-colors hover:bg-surface-container-low">
                    ...
                    <td className={`py-4 font-data-mono ${metric === "Em analise" ? "text-status-critical" : "text-status-success"}`}>{metric}</td>
                    <td className="py-4">
...
function DualBarChart() {
  return (
    <div className="mt-6 grid h-64 grid-cols-6 items-end gap-3 border-b border-outline/10 pb-2">
      {volumeTrend.slice(0, 6).map((value, index) => (
        <div key={`${value}-${index}`} className="flex h-full items-end gap-1">
          <div className="w-full rounded-t bg-secondary/70" style={{ height: `${value + 10}%` }} />
          <div className="w-full rounded-t bg-status-success/55" style={{ height: `${marginTrend[index] + 8}%` }} />
        </div>
      ))}
    </div>
  );
...
function CommercialMetric({ label, value, note, bordered = false, positive = false }: { label: string; value: string; note: string; bordered?: boolean; positive?: boolean }) {
  return (
...
function DistributionRow({ label, value, progress }: { label: string; value: string; progress: number }) {
  return (
    <div className="rounded-lg border border-glass-stroke bg-white/50 p-4">
      <div className="mb-3 flex items-center justify-between">
        <span className="font-label-caps text-[10px] text-outline">{label}</span>
        <span className="font-data-mono text-sm font-bold text-primary">{value}</span>
      </div>
      <ProgressBar value={progress} className="bg-secondary" />
    </div>
  );
}
