...
const totalRevenue = commercialRecords.reduce((total, record) => total + record.revenue, 0);
const totalUnits = commercialRecords.reduce((total, record) => total + record.units, 0);
const ticketAverage = totalRevenue / totalUnits;
...
const monthlyRevenue = commercialRecords.filter((record) => record.recurrence === "Mensal").reduce((total, record) => total + record.revenue, 0);
const monthlyShare = Math.round((monthlyRevenue / totalRevenue) * 100);
...
export const commercialAnalyticsView: CommercialAnalyticsView = {
  kpis: [
    { icon: "paid", label: "Faturamento", value: formatCurrency(totalRevenue), delta: "+8.4%", tone: "text-status-success", border: "border-secondary", progress: 82 },
  ...
  productRevenue: Object.entries(revenueByProduct).map(([label, value], index) => [
    label,
  ...
  customerRanking: commercialRecords.map((record) => [record.customer, formatCurrency(record.revenue), `${((record.revenue / totalRevenue) * 100).toFixed(1)}%`, record.recurrence]),
  openOrders: openCommercialPipeline.map((item) => [item.label, formatCurrency(item.value), item.progress, item.color]),
  recurrenceRows: [["Mensal", `${monthlyShare}%`, monthlyShare], ["Bimestral", "16%", 16], ["Anual", "0%", 0], ["Semestral", "0%", 0]],
  insights: [
    ...
    { icon: "monitoring", label: "Margin Alert", text: "Pedidos de terceiros devem ser avaliados contra margem media antes de comprometer capacidade produtiva.", tone: "text-status-critical", bg: "bg-error-container/40" },
    { icon: "warning", label: "Pipeline Risk", text: "Parte da meta depende de carteira aberta; usar status do pedido para separar confirmado de provisao.", tone: "text-status-critical", bg: "bg-status-critical/10" },
    { icon: "stars", label: "Strategic Highlight", text: `Clientes com periodicidade mensal concentram ${monthlyShare}% do faturamento da amostra comercial.`, tone: "text-status-success", bg: "bg-status-success/10" },
  ],
  tableRows: [
    [commercialRecords[0].customer, "Ranking Clientes", formatCurrency(commercialRecords[0].revenue), `${((commercialRecords[0].revenue / totalRevenue) * 100).toFixed(1)}%`, "Recorrente"],
    ["Big Bag Standard", "Tipo de Big Bag", formatCurrency(revenueByProduct["Big Bag Standard"] ?? 0), `${Math.round(((revenueByProduct["Big Bag Standard"] ?? 0) / totalRevenue) * 100)}%`, "Lider"],
    ...
    ["Portal Saint Gobain", "Orcamentos", "A definir", "Em analise", "Watch"],
  ],
};
