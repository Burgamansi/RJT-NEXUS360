...
export const operationsAnalyticsView: OperationsAnalyticsView = {
  kpis: [
    { icon: "precision_manufacturing", label: "Volume de produ??o", value: formatNumber(totalProduced), delta: `${formatNumber(totalProduced - totalPlanned)} vs plan`, tone: totalProduced >= totalPlanned ? "text-status-success" : "text-status-critical", border: "border-secondary", progress: Math.min(Math.round(operationalEfficiency), 100) },
  ...
  linePerformance: productionRecords.map((record) => [
    ...
    record.status === "bottleneck" ? "Bottleneck" : record.status === "watch" ? "Atenção" : "Optimal",
  ]),
  ...
  insights: [
    ...
    { icon: "timer_off", label: "Downtime Alert", text: `Paradas somam ${(totalDowntimeMinutes / 60).toFixed(1)}h na amostra operacional local.`, tone: "text-status-critical", bg: "bg-erro-container/40" },
  ...
  tableRows: [
    ...
    [productionRecords[0].machine, "Machine Logs", `${availability.toFixed(1)}%`, "Saudável", "Running"],
    [`${bottleneckLine?.line ?? "Linha"} Hold`, "Downtime Records", `${((bottleneckLine?.downtimeMinutes ?? 0) / 60).toFixed(1)}h`, "+18m", "Bottleneck"],
    ["Shift B Output", "Productivity Reports", `${Math.round(operationalEfficiency * 1.12)}`, "+8.7", "Adiantado"],
    ["Packaging Cell", "Operational History", `${oee.toFixed(1)}%`, "-2.1pp", "Atenção"],
  ],
};
